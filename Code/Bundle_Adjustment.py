...
def BundleAdjustment(points_3d, temp2, Rtnew, K, r_error):
	...
	opt_variables_init = np.hstack((Rtnew.ravel(), K.ravel()))
	opt_variables_2d = np.hstack((opt_variables_init, temp2.ravel()))				
	n2d = opt_variables_2d.shape[0] - opt_variables_init[0]
	opt_variables = np.hstack((opt_variables_2d, points_3d.ravel()))
	corrected_values = least_squares(fun = OptimReprojectionError, x0 = opt_variables, gtol = r_error).x
